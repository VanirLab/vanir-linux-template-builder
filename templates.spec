#
# This SPEC is for bulding RPM packages that contain complete Vanir Template files
# This includes the VM's root image, patched with all vanir rpms, etc
#
...
Name:		vanir-template-%{template_name}
Version:	%{version}
Release:	%{rel}
Summary:	Vanir template for %{template_name}
...
Requires(post):	tar
BuildArch:  noarch
Provides:	vanir-template
Obsoletes:  %{name} > %{version}-%{release}

%define _builddir %(pwd)
%define _rpmdir %(pwd)/rpm
%define dest_dir /var/lib/vanir/vm-templates/%{template_name}

%define _binaries_in_noarch_packages_terminate_build 0
%description
Vanir template for %{template_name}

%build
cd vanirized_images/%{template_name}
rm -f root.img.part.*
...
for i in vanirized_images/%{template_name}/root.img.part.* ; do mv $i $RPM_BUILD_ROOT/%{dest_dir}/ ; done
...
chown root.vanir %{dest_dir}/root.img
chmod 0660 %{dest_dir}/root.img

echo "--> Processing the volatile.img..."
/usr/lib/vanir/prepare-volatile-img.sh %{dest_dir}/volatile.img $[ `stat -c '%s' %{dest_dir}/root.img` / 1024 / 1024 ] || exit 1
chown root.vanir %{dest_dir}/volatile.img
chmod 0660 %{dest_dir}/volatile.img
tar --sparse -cf %{dest_dir}/clean-volatile.img.tar -C %{dest_dir} volatile.img
chown root.vanir %{dest_dir}/clean-volatile.img.tar
...
if [ "$1" = 1 ] ; then
    ...
    chown root.vanir %{dest_dir}/private.img
...
ln -sf /usr/share/vanir/icons/template.png %{dest_dir}/icon.png

local_user=`getent group vanir | cut -d : -f 4 | cut -d , -f 1`
...
if [ "`stat -c %d:%i /`" != "`stat -c %d:%i /proc/1/root/.`" ]; then
    ...
    call_as_user /usr/libexec/vanir-appmenus/create-apps-for-appvm.sh \
...
%files
%defattr(660,root,vanir,770)
%attr(2770,root,vanir) %dir %{dest_dir}
...
%attr (775,root,vanir) %dir %{dest_dir}/apps
%attr (775,root,vanir) %dir %{dest_dir}/apps.templates
%attr (775,root,vanir) %dir %{dest_dir}/apps.tempicons
%attr (664,root,vanir) %{dest_dir}/whitelisted-appmenus.list
%attr (664,root,vanir) %{dest_dir}/vm-whitelisted-appmenus.list
%attr (664,root,vanir) %{dest_dir}/netvm-whitelisted-appmenus.list
%{dest_dir}/icon.png
