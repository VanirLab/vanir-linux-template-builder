...
update-repo-templates.%:
	...
	ln -f rpm/noarch/vanir-template-$(TEMPLATE_NAME)-$(VERSION)-$(shell cat build_timestamp_$(TEMPLATE_NAME))*.noarch.rpm $$UPDATE_REPO/rpm

update-repo-installer:	
	[ -z "$$UPDATE_REPO" ] && UPDATE_REPO=../installer/yum/vanir-dom0;\
	ln -f rpm/noarch/vanir-template-$(TEMPLATE_NAME)-$(VERSION)-$(shell cat build_timestamp_$(TEMPLATE_NAME))*.noarch.rpm $$UPDATE_REPO/rpm

sign:
	setsid -w rpmsign $$RPMSIGN_OPTS --addsign rpm/noarch/vanir-template-$(TEMPLATE_NAME)-$(VERSION)-$(shell cat build_timestamp_$(TEMPLATE_NAME))*.noarch.rpm
...
clean:
	sudo rm -fr vanirized_images/root.img.*
	sudo rm -fr vanirized_images/$(TEMPLATE_NAME)*
	...
	# and they are not dependent on any of the Vanir packages
